...
> **本文是历史记录，结构部分已被同日稍晚的合仓取代。** 下面的仓库划分、mermaid 图和「无需先物理合仓」的判断，描述的是合仓之前的状态。Windows 端的 Server、GUI 框架、页面、安装器随后并入 MSIME-Windows，词库、辅助码、语音并入 MSIME-Engine，原仓库已归档为只读。当前的职责划分见 [AGENTS.md](AGENTS.md)。
>
