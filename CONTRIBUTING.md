...
本项目不要求签署 CLA，也不要求 commit 带 `Signed-off-by`。采用的是绝大多数开源项目的默认约定：**inbound = outbound**——你提交的贡献，以你提交到的那个仓库根目录 `LICENSE` 所声明的许可证授出，与该仓库既有代码完全一致。组织内多数仓库是 GPL-3.0，但不是全部（例如皮肤示例是 MIT，Google-PinyinIME-Rev 是 Apache-2.0），以你实际改动的那个仓库为准。

提交前请确认你有权以该许可证提供这些内容：代码是你自己写的，或者来源许可证与之兼容。**如果改动里包含从别处拿来的代码、词库、图标或字体，请在 PR 里写明来源和它的许可证**，不要默认它可以直接合进来——这一类问题在合并之后才发现，处理成本远高于提前问一句。
